// File: game.js
// (Versione finale completa con griglia 6x6 e piazzamento IA)
...
function handleHandCardClick(event) { if (gameState.gameEnded || gameState.currentPlayerId !== 1 || gameState.selectingTarget) return; const cardElement = event.currentTarget; const cardId = cardElement.dataset.cardId; const cardData = getCardData(cardId); const player = getPlayerState(1); if (!player || !cardData) return; console.log(`Tentativo di giocare: ${cardData.nome} (ID: ${cardId})`); if (player.currentResources < cardData.costo) { addLogMessage(`Risorse insufficienti per giocare ${cardData.nome} (costa ${cardData.costo}, hai ${player.currentResources}).`); console.log("Risorse insufficienti."); return; } const isCreature = cardData.forza !== null || cardData.punti_ferita !== null; let placementSpot = null; if (isCreature) { if (player.field.length >= gameState.maxCreaturesPerPlayer) { addLogMessage(`Non puoi giocare ${cardData.nome}: massimo numero di creature (${gameState.maxCreaturesPerPlayer}) raggiunto.`); console.log("Limite creature raggiunto."); return; } placementSpot = findEmptyPlacementSpot(player.id); if (!placementSpot) { addLogMessage(`Non puoi giocare ${cardData.nome}: nessuna posizione libera nella tua zona di schieramento (Fila ${gameState.grid.rows - 1}).`); console.log("Nessuna posizione libera per il piazzamento."); return; } } const hasTargeting = typeof cardData.getTargetInfo === 'function'; if (hasTargeting) { console.log(`${cardData.nome} richiede un bersaglio.`); player.currentResources -= cardData.costo; const cardIndex = player.hand.indexOf(cardId); if (cardIndex > -1) { player.hand.splice(cardIndex, 1); gameState.pendingActionInfo = { type: 'playCardFromHand', cardId: cardId, originalHandIndex: cardIndex, costPaid: cardData.costo }; console.log("Info per annullamento salvate:", gameState.pendingActionInfo); const context = createCardContext(player.id, cardId); const targetInfo = cardData.getTargetInfo(context); startTargetSelection(cardId, targetInfo); renderGame(); } else { console.error("Carta cliccata non trovata nella mano!", cardId, player.hand); player.currentResources += cardData.costo; } } else { console.log(`${cardData.nome} non richiede bersaglio o è una creatura piazzabile.`); player.currentResources -= cardData.costo; const cardIndex = player.hand.indexOf(cardId); if (cardIndex > -1) { player.hand.splice(cardIndex, 1); } else { console.error("Carta cliccata non trovata nella mano!", cardId, player.hand); player.currentResources += cardData.costo; return; } if (isCreature) { if (!placementSpot) { console.error("Errore logico: placementSpot non definito per creatura!"); addLogMessage(`! Errore piazzamento ${cardData.nome} !`); } else { const instanceId = generateInstanceId(player.id, cardId); const newCreature = { cardId: cardId, instanceId: instanceId, position: placementSpot, currentHp: cardData.punti_ferita, canAttackThisTurn: !cardData.keywords?.includes('Lento'), ownerId: player.id }; player.field.push(newCreature); addLogMessage(`Giocatore ${player.id} schiera ${cardData.nome} in Fila ${placementSpot.row}, Colonna ${placementSpot.col}.`); executeEffect(cardId, 'onPlay', player.id, { instanceId: instanceId }); } } else { addLogMessage(`Giocatore ${player.id} lancia ${cardData.nome}.`); executeEffect(cardId, 'onPlay', player.id); player.graveyard.push(cardId); } renderGame(); } }
...
function runOpponentTurn() {
    ...
    // --- Funzione IA per giocare una carta ---
    const playCardAI = () => {
        let cardPlayed = false;
        // Trova carte giocabili (costo <= risorse)
        const playableCards = player.hand
            ...
            .sort((a, b) => b.data.costo - a.data.costo); // Prova a giocare prima le più costose

        for (const cardToPlay of playableCards) { // Itera sulle carte giocabili
            const cardId = cardToPlay.id;
            const cardData = cardToPlay.data;
            const isCreature = cardData.forza !== null || cardData.punti_ferita !== null;

            // Salta carte con target (IA semplice)
            ...
            // --- Se siamo qui, la carta è giocabile ---
            console.log(`IA: Gioca ${cardData.nome}`);

            // Paga costo e rimuovi da mano
            player.currentResources -= cardData.costo;
            // Trova indice CORRETTO nella mano attuale (potrebbe essere cambiata da iterazioni precedenti)
            const currentHandIndex = player.hand.indexOf(cardId);
             if (currentHandIndex > -1) {
                 ...
                 player.currentResources += cardData.costo; // Rollback costo
                 continue; // Errore strano, prova prossima carta
             }


            // Piazza creatura o lancia magia
            if (isCreature) {
                 if (placementSpot) { // Dovrebbe sempre essere vero se siamo arrivati qui
            ...
            cardPlayed = true;
            renderGame(); // Aggiorna UI dopo ogni carta giocata
            break; // L'IA gioca solo UNA carta per turno in questo esempio
        } // Fine loop carte giocabili

        return cardPlayed;
    }; // Fine playCardAI

    // --- Funzione IA per attaccare ---
    const attackAI = () => {
        let didAttack = false;
        ...
        if (attackers.length > 0) {
            // IA Semplice: attacca giocatore con la prima creatura
            const attackerInstance = attackers[0];
            const attackerData = getCardData(attackerInstance.cardId);
            const damage = attackerData.forza;

            if (opponent) { // Controlla che l'avversario esista
                opponent.hp -= damage;
                attackerInstance.canAttackThisTurn = false; // Ha attaccato
    ...
    // --- Esecuzione turno IA con ritardi ---
    setTimeout(() => {
        if (gameState.gameEnded || gameState.currentPlayerId !== 2) return;
        const played = playCardAI();
        if (!played) { addLogMessage("Giocatore 2 non gioca carte questo turno."); }

        setTimeout(() => {
            if (gameState.gameEnded || gameState.currentPlayerId !== 2) return;
            const attacked = attackAI();
            // Non logghiamo se non attacca
...
function addEventListeners() {
    ...
    // Nota: Listener per celle griglia vuote aggiunto in renderField
    // Nota: Listener per carte in mano/campo aggiunto in renderCard
}
...
function initGame() {
    ...
    addEventListeners();
    startTurn(1); // Avvia il primo turno dopo l'init
}
