/* File: game.js */
/* (Versione finale - Reinserita renderEcoDots) */
...
const ecoDotsP1 = document.getElementById('eco-dots-1'); /* Aggiunto riferimento */
...
const ecoDotsP2 = document.getElementById('eco-dots-2'); /* Aggiunto riferimento */
...
const player1InfoArea = document.getElementById('player-1-info-area'); /* Contenitore info G1 */
const player2InfoArea = document.getElementById('player-2-info-area'); /* Contenitore info G2 */
...
function renderCard(cardId, location = 'hand', instanceId = null) { /* ... Definizione Completa ... */ }
function renderHand(playerId) { /* ... Definizione Completa ... */ }
function renderField() { /* ... Definizione Completa ... */ }

/** NUOVA Funzione Helper: Renderizza i pallini dell'Eco */
function renderEcoDots(containerElement, current, max) {
    if (!containerElement) return;
    containerElement.innerHTML = ''; /* Pulisci precedente */
    const maxVisualDots = 10; /* Mostra sempre 10 pallini */
    for (let i = 0; i < maxVisualDots; i++) {
        const dot = document.createElement('div');
        dot.classList.add('eco-dot');
        if (i < current) {
            dot.classList.add('active'); /* Eco disponibile */
        } else if (i < max) {
            dot.classList.add('max'); /* Slot Eco massimo ma non pieno */
        }
...
function renderPlayerInfo(playerId) {
    ...
    renderEcoDots(ecoDotsEl, playerState.currentResources, playerState.maxResources); /* Chiama la funzione helper */
